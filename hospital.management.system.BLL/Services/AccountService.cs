...
public class AccountService : IAccountService
{
    ...
    public async Task<Result<string, DomainError>> ResetPasswordAsync(ResetPasswordModel model)
    {
        ...
        return result.Succeeded
            ...
            : Result.Failure<string, DomainError>(new DomainError("Something wrong has happened"));
    ...
    public async Task<Result<GetAuthenticatorKeyResponseModel, DomainError>> GetAuthenticatorTokenAysnc(
        GetAuthenticatorTokenModel model)
    {
        string AuthenticatorUriFormat = "otpauth://totp/{0}:{1}?secret={2}&issuer={0}&digits=6";

        var user = await userManager.FindByNameAsync(model.UserName ?? "");
        if (user == null)
            return Result.Failure<GetAuthenticatorKeyResponseModel, DomainError>(
                new DomainError("Something wrong has happened "));
    ...
    public async Task<Result<TwoFactorAuthenticationResponseModel, DomainError>> EnableTwoFactorAuthenticationAsync(
        TwoFactorAuthenticationModel model)
    {

        var user = await userManager.FindByNameAsync(model.UserName ?? "");
        if (user == null)
            return Result.Failure<TwoFactorAuthenticationResponseModel, DomainError>(
                new DomainError("Something wrong has happened "));

        bool succeeded = await userManager.VerifyTwoFactorTokenAsync(user,
            userManager.Options.Tokens.AuthenticatorTokenProvider, model.Code);

        if (!succeeded)
            return Result.Failure<TwoFactorAuthenticationResponseModel, DomainError>(
                new DomainError("Verify ,Your two factor auth code could not be validated."));
    ...
    public async Task<Result<VerifyAuthenticatorResponseModel>> TwoFactorAuthenticatorSignInAsync(
        VerifyAuthenticatorModel model)
    {
